...
suma_wyslanych_paczek = 0
waga_elementu_w_paczce = 0
waga_paczki = 0

liczba_paczek = int(input("Podaj liczba_paczek: "))

for numer_paczki in range(liczba_paczek):
    waga_elementu_w_paczce = float(input(f"Podaj wage {numer_paczki + 1} (w kg): "))
    if waga_elementu_w_paczce< 1 or waga_elementu_w_paczce > 10:
        print(f"Podana waga paczki jest spoza przedzialu 1 - 10 kg")
        break

    if calkowita_waga_paczek + waga_elementu_w_paczce > 20:
        aktualna_ilosc_paczek += 1
        suma_wyslanych_paczek += 1
        calkowita_waga_paczek = calkowita_waga_paczek + waga_elementu_w_paczce
    else:
        waga_paczki += waga_elementu_w_paczce

suma_pustych = calkowita_waga_paczek * 20 - aktualna_ilosc_paczek
